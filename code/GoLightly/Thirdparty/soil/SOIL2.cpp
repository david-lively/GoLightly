...
#ifndef GL_GLEXT_PROTOTokenType
#define GL_GLEXT_PROTOTokenType
#endif
#ifdef SOIL_PLATFORM_IOS
 ...
 )
{
    ...
    // DL: initialize width & height to avoid OSX warning
...
void check_for_GL_errors( const char *calling_location )
{
	...
	while( GL_NO_ERROR != err_code )
	{
		...
		printf( "OpenGL Error @ %s: %i \"%s\"\n", calling_location, err_code, error );
		err_code = glGetError();
	}
}
#else
 ...
 )
{
	...
	if( tex_id )
	{
		...
		{
            //            glTexImage2D(<#GLenum target#>, x, <#GLint internalformat#>, <#GLsizei width#>, <#GLsizei height#>, <#GLint border#>, <#GLenum format#>, <#GLenum type#>, <#const GLvoid *pixels#>)
			/*	user want OpenGL to do all the work!	*/
            
			check_for_GL_errors( "pre-glTexImage2D" );
