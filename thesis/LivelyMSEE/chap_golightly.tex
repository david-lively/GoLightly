...
A completed PBO is bound to a uniform input for a shader (\autoref{lst:basicvert}, \autoref{lst:basicfrag}) listing which renders the texture to the visualizer window. This process runs continuously. In order to reduce GPU overhead, the texture is only updated at the frequency requested by the user when the simulation was launched.
