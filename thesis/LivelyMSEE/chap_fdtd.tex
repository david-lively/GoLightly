...
In equations \ref{eq:ezupdate}, \ref{eq:hxupdate} and \ref{eq:hyupdate}, note that each $E$ field update depends only upon the previous $E$ value ($E_Z^{t}$), and the previous adjacent $H$ values ($H_X^{t+\frac{1}{2}}$ and $H_Y^{t+\frac{1}{2}}$. This allows each field component to be updated without regard for any other value in the same field. This allows the simulator to update all $E$ field values in any order without the risk of a race condition. This also applies to $H$-field calculations. 

\subsection{Leap Frog: Stepping in Space and Time}

In equations \ref{eq:ezupdate}, \ref{eq:hxupdate} and \ref{eq:hyupdate},
